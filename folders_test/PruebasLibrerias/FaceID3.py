...
def draw_label(frame, text, x, y):
    ...
    x_center = x + (size[0] // 2)
    y_center = y - 10
    cv2.putText(frame, text, (x_center, y_center), font, scale, color, thickness, cv2.LINE_AA)
...
while True:
            ...
            for fid, tracker in trackers.items():
                ...
                if success:
                    ...
                    if abs(x - tx) < w and abs(y - ty) < h:
                        ...
                        break
            if not matched:
                # Usar el método actualizado para crear un rastreador CSRT
    ...
    # Mostrar el frame
    cv2.imshow("Detect Camera", frame)
